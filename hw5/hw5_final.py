...
k = 7 # max cycle fox can go without food

# creating parser to integrate arguments into command line
parser = argparse.ArgumentParser(
                    prog='Rab vs Fox',
                    description='A simulation of Foxes and Rabbits')

# adjusts grass regrowth rate
parser.add_argument('grass_rate', help='The probability that grass grows back', type=float, const=0.9,nargs='?')

# adjusts fox life expectancy without eating
parser.add_argument('fox_k_val', help='The number of cycles a fox can survive without food', type=int, const=10, nargs='?')

# adjusts size of field
parser.add_argument('size', help='The side length of the square field to be used in the simulation', type=int, const=50, nargs='?')

# adjusts the starting number of rabbits
parser.add_argument('r_init', help='The initial number of rabbits in the field', type=int, const=200, nargs='?')

# adjusts the starting number of foxes
parser.add_argument('f_init', help='The initial number of foxes in the field', type=int, const=50, nargs='?')

args = parser.parse_args()

# integrating command line arguments
if args.grass_rate:
...
if args.r_init:
    INIT_RABBITS = args.r_init

if args.f_init:
    INIT_FOXES = args.f_init
